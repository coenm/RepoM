...
[UsedImplicitly]
internal class ActionOpenHeidiV1Deserializer : IActionDeserializer
{
    public bool CanDeserialize(string type)
    {
        return "open-heidi-db@1".Equals(type, StringComparison.CurrentCultureIgnoreCase);
    }
    ...
    private static RepositoryActionOpenHeidiV1? Deserialize(JToken jToken, JsonSerializer jsonSerializer)
    {
        RepositoryActionOpenHeidiV1? result = jToken.ToObject<RepositoryActionOpenHeidiV1>(jsonSerializer);
