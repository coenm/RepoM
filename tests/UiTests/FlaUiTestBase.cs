...
/// This class allows recording videos, taking screenshots on failed tests and
...
public abstract class FlaUiTestBase
{
    ...
    /// </summary>
    protected abstract string TestsMediaPath
    {
        get;
        // {
        //     return $@"c:\temp\testsmedia\{SanitizeFileName(TestContext.CurrentContext.Test.Name)}\{_testDateTime}";
        // }
    }
    ...
    public virtual async Task UITestBaseOneTimeSetUp()
    {
        ...
        {
            if (SetTestNameAttribute.TryGet(out var info))
            {
                await StartVideoRecorder(SanitizeFileName(info.Name));
            }
            else
            {
                await StartVideoRecorder(SanitizeFileName("SETUP"));
            }
        }
    ...
    public virtual async Task UITestBaseSetUp()
    {
        ...
        if (SetTestNameAttribute.TryGet(out var info))
        {
            _testMethodName = info.Name;
        }
        else
        {
            _testMethodName = "SETUP";
        }
        

        if (VideoRecordingMode == VideoRecordingMode.OnePerTest)
        {
            await StartVideoRecorder(SanitizeFileName(_testMethodName));
        }
    ...
    /// <summary>
    /// Method which captures the image for the video and screenshots.
