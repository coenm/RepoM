...
public class GitFetchV1Test
{
    ...
    {
        _sut = DynamicRepositoryActionDeserializerFactory.CreateWithDeserializer(new DefaultActionDeserializer<RepositoryActionGitFetchV1>());
    ...
    [Fact]
    public async Task Deserialize_GitFetchV1()
    {
        // arrange
    ...
    {
        // arrange
        _testFileSettings.UseMethodName(nameof(Deserialize_GitFetchV1));
        var content = await EasyTestFile.LoadAsText(_testFileSettings);
        ...
        // assert
        _ = result.ActionsCollection.Actions.Should().AllBeOfType<RepositoryActionGitFetchV1>();
    }
}