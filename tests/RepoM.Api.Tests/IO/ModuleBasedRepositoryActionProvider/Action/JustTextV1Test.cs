...
public class JustTextV1Test
{
    ...
    public JustTextV1Test()
    {
        _sut = DynamicRepositoryActionDeserializerFactory.CreateWithDeserializer(new DefaultActionDeserializer<RepositoryActionJustTextV1>());
    ...
    [Fact]
    public async Task Deserialize_JustText1()
    {
        // arrange
    ...
    {
        // arrange
        _testFileSettings.UseMethodName(nameof(Deserialize_JustText1));
        var content = await EasyTestFile.LoadAsText(_testFileSettings);
        ...
        // assert
        _ = result.ActionsCollection.Actions.Should().AllBeOfType<RepositoryActionJustTextV1>();
    }
}