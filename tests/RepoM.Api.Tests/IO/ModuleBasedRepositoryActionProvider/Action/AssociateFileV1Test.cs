...
public class AssociateFileV1Test
{
    ...
    {
        _sut = DynamicRepositoryActionDeserializerFactory.CreateWithDeserializer(new DefaultActionDeserializer<RepositoryActionAssociateFileV1>());
    ...
    [Fact]
    public async Task Deserialize_AssociateFile1()
    {
        // arrange
    ...
    {
        // arrange
        _testFileSettings.UseMethodName(nameof(Deserialize_AssociateFile1));
        var content = await EasyTestFile.LoadAsText(_testFileSettings);
        ...
        // assert
        _ = result.ActionsCollection.Actions.Should().AllBeOfType<RepositoryActionAssociateFileV1>();
    }
}